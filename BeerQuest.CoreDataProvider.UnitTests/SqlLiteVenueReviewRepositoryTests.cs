...
public class SqlLiteVenueReviewRepositoryTests
{
    ...
    [Test]
    [AutoData]
    public async Task GetVenueReviewsDateTimeAsync_CallsQueryExecutor_WithCorrectQuery(DateTime dateTime,
    ...
    [Test]
    [AutoData]
    public async Task GetVenueReviewsByRatingAsync_CallsQueryExecutor_WithCorrectQuery(RatingType ratingType,
        double rating, ComparisonOperator comparisonOperator)
    {
        var expectedQuery =
            BaseQuery + $"WHERE {ratingType.ToDbColumn()} {comparisonOperator.ToOperatorString()} @rating";
    ...
    {
        var expectedQuery = BaseQuery +
                            "WHERE tags LIKE '%' || @param0 || '%' AND tags LIKE '%' || @param1 || '%' AND tags LIKE '%' || @param2 || '%'";
